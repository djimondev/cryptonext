...
function NavigationCard({ title, description, icon, onClick }: NavigationCardProps) {
  return (
    ...
    >
      <div className="p-4 bg-primary-50 dark:bg-gray-700 rounded-full">{icon}</div>
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center p-6">
      <div className="max-w-4xl w-full mx-auto space-y-8">
        <h1 className="text-3xl md:text-4xl font-bold text-center text-gray-900 dark:text-white mb-8">{t("home.title")}</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {navigationOptions.map(option => (
            <NavigationCard key={option.title} title={option.title} description={option.description} icon={option.icon} onClick={() => navigate(option.path)} />
          ))}
        </div>
      </div>
    </div>
